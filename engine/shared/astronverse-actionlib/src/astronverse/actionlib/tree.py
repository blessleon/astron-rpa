...
class TreeNode:
    ...
    def __init__(self, key: str, title: str = "", icon: str = "", extend: bool = False):
        self.key = key
        self.title = title
        self.icon = icon
        self.extend = extend  # 是否为扩展节点
        self.atomics = []
        self.children = {}
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # 优先添加原子能力（叶子节点）
        result["atomics"].extend(self.atomics)
        
        # 然后添加子节点（树节点）
        for child_key, child_node in self.children.items():
            result["atomics"].append(child_node.to_dict())
...
class TreeManager:
    ...
    def _get_or_create_node(self, path: str) -> TreeNode:
        ...
        icon = node_info.get("icon", key)
        extend = node_info.get("extend", False)  # 读取 extend 属性
        
        # 如果是根节点
        if len(parts) == 1:
            if key not in self.root_nodes:
                self.root_nodes[key] = TreeNode(key, title, icon, extend)
        ...
        if key not in parent_node.children:
            parent_node.add_child(key, TreeNode(key, title, icon, extend))
    ...
    def to_dict(self) -> Dict[str, Any]:
        """转换为字典，区分 atomicTree 和 atomicTreeExtend"""
        atomic_tree = []
        atomic_tree_extend = []
        
        for root_key, root_node in self.root_nodes.items():
            node_dict = root_node.to_dict()
            if root_node.extend:
                atomic_tree_extend.append(node_dict)
            else:
                atomic_tree.append(node_dict)
